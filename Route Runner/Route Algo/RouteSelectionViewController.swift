...
//  From tutorial at  https://www.raywenderlich.com/10028489-routing-with-mapkit-and-core-location
...
class RouteSelectionViewController: UIViewController {
  ...
  private func attemptLocationAccess() {
    // make sure user has enabled location services
    guard CLLocationManager.locationServicesEnabled() else {
      return
    }
    // set accuracy level
    locationManager.desiredAccuracy = kCLLocationAccuracyHundredMeters
    // manages location and privacy changes
    locationManager.delegate = self
    // request the current location of user if privacy allows
